...
class AccessChecker @Inject()(config: Configuration) extends ExtendedServicesConfig(config):
  ...
  private val allowedClients: Set[String] = getStringSeq(accessControlAllowListKey,
    if checkAllowList then throw new RuntimeException(s"Could not find config $accessControlAllowListAbsoluteKey") 
    else Seq()).toSet
  ...
  def getClientsFromRequest[T](req: Request[T]): Seq[String] = 
    ...
    if originator.isDefined then Seq(originator.get)
    else req.headers.getAll(HeaderNames.USER_AGENT).flatMap(_.split(","))
